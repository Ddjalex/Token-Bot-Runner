...
router.post("/notify-withdrawal-status", async (req, res) => {
  try {
    ...
    if (transaction_number && status === "rejected") {
      message += `Rejected Reason: ${transaction_number}\n`;
    }
    ...
    // Use the safe send message function
    const result = await safeSendMessage(bot, telegram_id, message, {
      parse_mode: "Markdown",
    });
    ...
    if (result) {
      ...
      res.json({
        success: true,
        message: "User may have blocked the bot, but request processed",
      });
    }
  } catch (error) {
